...
test('Edit Circular Geofence', async ({ page }) => {
  ...
  // Click on save button plotted on the map
  ...
  try {
    await page.locator('#saveConfirmation').waitFor({ state: 'visible', timeout: 10000 });
    ...
    try {
      await page.evaluate(() => {
        ...
        const modal = document.getElementById('saveConfirmation');
        if (modal) {
      ...
      await page.waitForTimeout(2000);
      const isModalVisible = await page.locator('#saveConfirmation').isVisible();
      if (isModalVisible) {
      ...
      try {
        await page.evaluate(() => {
          ...
          if (window.$ && window.$('#saveConfirmation').length) {
            window.$('#saveConfirmation').modal('show');
            return true;
          }
        ...
        await page.waitForTimeout(3000);
        const isModalVisible2 = await page.locator('#saveConfirmation').isVisible();
        if (isModalVisible2) {
  ...
  try {
    ...
    const confirmButton = page.locator('#saveConfirmation .btn-confirm');
    ...
    try {
      const alternativeConfirmSelectors = [
        page.locator('#saveConfirmation button:has-text("Confirm")'),
        page.locator('#saveConfirmation .btn-confirm'),
        page.locator('#saveConfirmation .btn-success'),
        page.locator('#saveConfirmation button[type="submit"]')
      ];
