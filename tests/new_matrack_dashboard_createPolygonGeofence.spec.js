...
test('Create Polygon Geofence', async ({ page }) => {
  ...
  console.log('Attempting to navigate to Create Polygon Geofence...');
  ...
  console.log('Clicking on Create Polygon Geofence...');
  const createPolygonGeofenceSelectors = [
    page.locator('#aPolygonGeofenceCreate').first(),
  ...
  for (const selector of createPolygonGeofenceSelectors) {
    try {
      await selector.waitFor({ state: 'visible', timeout: 5000 });
      await selector.click();
      console.log('Successfully clicked on Create Polygon Geofence');
  ...
  if (!createPolygonGeofenceClicked) {
    throw new Error('Could not find or click Create Polygon Geofence link');
  }

  // Wait for Create Polygon Geofence page to load
  console.log('Waiting for Create Polygon Geofence page to load...');
  await page.waitForLoadState('domcontentloaded');
  await page.waitForTimeout(5000);

  // adding details to create LandMark
  await page.locator('#divCreatePolygonGeofence .txtCreateGeofenceAddress').fill('San Diego, CA, United States');
  ...
  await page.locator('#divCreatePolygonGeofence .txtCreateGeofenceRadius').fill('350');
  await page.locator('#divCreatePolygonGeofence .txtCreateGeofenceName').fill('Test Polygon 1');

  await page.waitForTimeout(2000);
  // click on checkbox to select the LandMark under inventory
  await page.locator('#divCreatePolygonGeofence .txtCreateGeofenceSides').fill('5');

  await page.waitForTimeout(2000);
  // Generate LandMark Circle on map
  await page.locator('#divCreatePolygonGeofence #btnCreatePolygonGeofence').click();
  ...
  try {
    await page.locator('#saveConfirmationPolygon').waitFor({ state: 'visible', timeout: 10000 });
    ...
    try {
      await page.evaluate(() => {
        ...
        const modal = document.getElementById('saveConfirmationPolygon');
        if (modal) {
      ...
      await page.waitForTimeout(2000);
      const isModalVisible = await page.locator('#saveConfirmationPolygon').isVisible();
      if (isModalVisible) {
      ...
      try {
        await page.evaluate(() => {
          ...
          if (window.$ && window.$('#saveConfirmationPolygon').length) {
            window.$('#saveConfirmationPolygon').modal('show');
            return true;
          }
        ...
        const isModalVisible2 = await page.locator('#saveConfirmationPolygon').isVisible();
        if (isModalVisible2) {
  ...
  try {
    ...
    const confirmButton = page.locator('#saveConfirmationPolygon .btn-confirm');
    ...
    try {
      const alternativeConfirmSelectors = [
        page.locator('#saveConfirmationPolygon button:has-text("Confirm")'),
        page.locator('#saveConfirmationPolygon .btn-confirm'),
        page.locator('#saveConfirmationPolygon .btn-success'),
        page.locator('#saveConfirmationPolygon input[type="submit"]'),
        page.locator('#saveConfirmationPolygon button[type="submit"]')
      ];
